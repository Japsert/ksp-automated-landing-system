...
for line in lines:
    ...
    cols = line.split(",")
    runs[-1].append((float(cols[time_col]), float(cols[lat_col]), float(cols[lng_col])))

start = time()
data_start_time = min(run[0][time_col] for run in runs)
log_time_range = max(run[-1][time_col] for run in runs) - data_start_time

fig, ax = plt.subplots()

# Calculate center and bounds
center_lat = runs[0][-1][lat_col]  # take first run's last point as center
center_lng = runs[0][-1][lng_col]
...
ax.legend(lines, [f"Run {i}" for i in range(len(runs))])


def get_values_until_time(time):
    # Return list of runs, containing lists of points, until the given time
    return_runs = []
    for run in runs:
        return_runs.append([])
        for point in run:
